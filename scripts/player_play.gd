...
var current_frame: int = 4  # Start with frame 4
...
func _on_toggle_timer_timeout():
	if is_interacting and bed != null:
		current_frame = 5 if current_frame == 4 else 4  # Toggle frames
		bed.frame = current_frame
...
func _input_event(viewport, event, shape_idx: int) -> void:
	if event is InputEventMouseButton:
		if event.pressed:
			start_frame_4_5()  # Start toggling frames 4 and 5
		else:
			stop_frame_4_5()  # Stop toggling
